...
pipeline benchmark use this boundary to replace the model with
`ScriptedEstimator`.
...
At EOF or reset, even a segment shorter than the FIR group delay receives the
full held-endpoint padding before its first packet is resolved. This keeps
short-segment output equivalent to centered FIR convolution with edge padding
for orientation, center, and apparent size.
