...
The effective Screen Age is the smallest `k` that minimizes the equal-subject
mean of subject-level squared distances between corresponding corners of
`F(t)` and `Q_k(t)`. Within each subject, squared error is pooled across retained
frames. Holding `C(t)` fixed prevents face translation and apparent-scale
changes from being misidentified as attitude-filter delay.
...
The model-free benchmark applies the same corner-matching idea to two frozen
synthetic motions for Raw, EMA, FIR, One Euro, and Kalman. Those workload-specific
ages support a controlled comparison; they do not replace this development and
held-out calibration protocol.

## Rebuild the figure

**Reproduction scope:** this checkout contains the aggregate receipt and figure
generator. The BIWI data-to-result runners currently live in a separate local
experiment workspace and are not shipped here. The command below rebuilds the
figure from saved evidence; it does not rerun the study. Publishing portable
study code and a data-to-receipt command remains required for independent
end-to-end reproduction. Preserve that experiment's source and detailed
receipts until this migration is complete.
...
consistent browser rendering at a centered 640-pixel display width; the preview
