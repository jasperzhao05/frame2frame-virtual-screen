...
It does include short, attributed derived media assets described below.
...
### Recorded application-content illustration

`docs/demo-live-desmos.gif` combines actual application captures with a
prerecorded person:

- Application: [Desmos Graphing Calculator](https://www.desmos.com/calculator),
  captured from its running browser interface on 2026-09-09. The example uses
  `y = 2 sin(x - a)` and `y = cos(2x + a)`, with Desmos animating `a` between
  -10 and 10. No account or private graph is used. The Desmos interface and
  marks remain the property of their respective rights holders and are not
  relicensed under this repository's MIT license.
- Person: 00:43.000–00:49.000 of Intel IoT DevKit's
  [`head-pose-face-detection-female.mp4`](https://github.com/intel-iot-devkit/sample-videos/blob/master/head-pose-face-detection-female.mp4),
  under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/). The source
  digest is listed above. The excerpt is muted and receives a virtual-screen
  overlay and a fixed presentation resize, with no source crop.

The capture contains 381 browser screenshots over approximately 12 seconds at
1280×720. The six-second illustration samples the latest completed screenshot
at or before each zero-based person-video timestamp; screenshot completion
times are rebased to the first capture. The left reference pane and projected
screen use the same sampled image. The 72 selected application images are
distinct. No synthetic UI frames or frame interpolation are used.

The public file-processing pipeline runs MediaPipe canonical-face pose fitting
and the live example's Kalman configuration: attitude only, acceleration
standard deviation 100 degrees/s², measurement standard deviation 1 degree.
Screen distance is 4, width/height multipliers are 4/2.25, opacity is 0.9, and
`contain` preserves the application's aspect ratio. Delay compensation and
audio are disabled. Fresh face observations occur on 67 of 72 frames; the
existing short-gap policy handles the other five, and all 72 frames are kept.
These counts describe this clip, not pose accuracy or general robustness.

The 960×312, 256-color GIF presents 72 frames at a nominal 12 fps and original
playback speed. An earlier 00:32–00:42 cut was replaced after visual review
because its longer front-facing intervals obscured the face behind the screen;
the rendering configuration was unchanged. No per-frame reframing, manual
screen keyframes, reversed footage, or slow motion is used.

This is recorded replay through a timestamp-aware content callback. It
illustrates application-content compositing; it is not a webcam or
`LatestFrameSource` integration test, an end-to-end latency measurement, or
headset output. Raw captures, full source footage, and local build records are
not committed. This project is not affiliated with or endorsed by Desmos or
Intel.
...
`docs/demo-comparison.gif` and everything produced by
`scripts/make_showcase.py` are generated from project-authored shapes, signals,
and textures. They contain no person footage, third-party image, or model
output and are covered by the repository's MIT License.
