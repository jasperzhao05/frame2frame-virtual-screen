...
It does include two short, attributed derived GIFs described below. Optional
...
`docs/screen-age-scene.jpg` is a 420×420 still from the same fixed-crop,
globally light-adjusted BooredAtWork excerpt. `docs/screen-age.svg` embeds those
exact verified JPEG bytes as the real-scene context for Panel A. The current
screen outline and sparse earlier-position trail are project-authored
explanatory overlays; they illustrate estimated FIR output history and are not
Rokid device output, BIWI evaluation imagery, or record-level benchmark
evidence. The still and subject likeness remain under CC BY 3.0 rather than the
...
and its PNG preview use only this aggregate receipt for Panel B. Their
real-person Panel A is the separately attributed CC BY 3.0 Rokid-review still
described above, not BIWI imagery.
