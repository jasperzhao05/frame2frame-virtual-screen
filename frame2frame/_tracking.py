...
class _TemporalTracker:
    ...
    def _flush_segment(self) -> list[_PendingFrame]:
        ...
        if not self.delayed:
            return []
        # A segment shorter than the delay has never filled the queue. Advance
        # the held endpoint until its first packet reaches the same filter age
        # as in normal streaming, then resolve the tail in source order.
        for _ in range(max(0, self.latency - len(self.delayed))):
            self._filter_step(self._held_sample(), 1.0 / self.fps)
        ready = []
        while self.delayed:
