...
At each output frame `t`, the protocol compares the filter output with screens
projected from earlier ground-truth attitudes while keeping the current center
and apparent scale fixed. The Screen Age is the smallest age that minimizes
pooled squared corner error across all retained development frames; it is
