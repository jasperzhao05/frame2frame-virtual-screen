...
Temporal smoothing is not free: a steadier screen can represent an older
moment. **Screen Age** measures that shift in projected geometry by matching
the current filtered screen to counterfactual screens built from past
orientations with the current center and scale.

[![Screen Age mechanism schematic beside a calibration plot of analytic FIR delay against recovered Screen Age](docs/screen-age.png)](docs/screen-age.svg)

*Projected Screen Chronometry. Panel A schematically illustrates the default
FIR response at 30 fps: its current geometry is closest to the counterfactual
screen from 11 frames, or 367 ms, earlier. Panel B aggregates 45 continuous
BIWI segments from 20 subjects; all six linear-phase FIR designs recover their
analytic 7-, 11-, or 19-frame delay in both the development estimate and
held-out diagnostic. Screen Age isolates attitude-filter response under a
shared current center and scale; it is not device or end-to-end latency.*

Read the [protocol and reproducible figure receipt](docs/SCREEN_AGE.md) for the
formal definition, split discipline, bootstrap audit, and scope limits.
...
scripts/
  ...
  make_screen_age_figure.py deterministic figure from a compact aggregate receipt
