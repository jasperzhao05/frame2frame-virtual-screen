...
from frame2frame.filters import FIRAttitudeFilter
...
@pytest.mark.parametrize("length", [1, 2, 3, 5, 10, 11, 12, 25])
@pytest.mark.parametrize("end_segment", [False, True], ids=["eof", "reset"])
def test_fir_segment_tail_matches_centered_convolution(length, end_segment):
    """Short segments must mature as far as long ones before being emitted."""
    cfg = PipelineConfig(input="unused.mp4", filter=FilterConfig())
    smoother = FIRAttitudeFilter(30.0, cfg.filter)
    tracker = _TemporalTracker(cfg, 30.0, smoother, _AngleDiagnostics(False, 50))
    values = np.column_stack(
        [
            np.linspace(start, stop, length)
            for start, stop in [(0, 20), (0, 8), (0, 4), (100, 130), (80, 95), (30, 40)]
        ]
    )
    packets = []
    for index, sample in enumerate(values):
        observation = FaceObservation(
            HeadPose(*sample[:3]), tuple(sample[3:5]), sample[5], (0.0, 0.0, 1.0, 1.0)
        )
        packets.extend(
            tracker.push(np.zeros((2, 2, 3), np.uint8), observation, index, index / 30.0)
        )
    packets.extend(tracker._end_segment() if end_segment else tracker.finish())
    actual = np.array(
        [
            [
                p.observation.pose.yaw,
                p.observation.pose.pitch,
                p.observation.pose.roll,
                *p.observation.center,
                p.observation.size,
            ]
            for p in packets
        ]
    )
    delay = smoother.group_delay
    expected = np.column_stack(
        [
            np.convolve(
                np.pad(values[:, axis], (delay, delay), mode="edge"),
                smoother._taps[channel],
                mode="valid",
            )
            for axis, channel in enumerate(("yaw", "pitch", "roll", "cx", "cy", "size"))
        ]
    )
    np.testing.assert_allclose(actual, expected, atol=1e-10)
    assert [p.frame_index for p in packets] == list(range(length))
    assert [p.media_time_seconds for p in packets] == pytest.approx(np.arange(length) / 30)
...
def test_delay_compensation_advances_on_every_video_frame(monkeypatch, tmp_path):
    ...
    patch_video_io(monkeypatch, reader)
    monkeypatch.setattr(
        pipeline.render,
        "draw_pose_axis",
...
def test_sparse_detections_keep_wall_clock_delay_and_queue_bounded(monkeypatch, tmp_path):
    ...
    patch_video_io(monkeypatch, reader, writer=Writer)
