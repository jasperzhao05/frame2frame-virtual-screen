...
@pytest.mark.parametrize(
    ("key", "value", "message"),
    [
        pytest.param("subject_id", "F01", "record-level field", id="subject-id"),
        pytest.param(
            "note",
            "/Users/private/frame.jpg",
            "local or media reference",
            id="local-path",
        ),
    ],
)
def test_public_receipt_rejects_record_level_data(key, value, message):
    receipt = deepcopy(_receipt())
    receipt["private"] = {key: value}
