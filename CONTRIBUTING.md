...
Run both deterministic benchmarks:
...
| Geometry | `python -m pytest -q tests/test_geometry.py` | render tests and pipeline benchmark |
| Texture or compositing | `python -m pytest -q tests/test_textures.py tests/test_render.py` | geometry tests and pipeline benchmark |
...
- Do not include real-person footage unless distribution and subject-consent
  rights are explicit. Prefer project-authored synthetic fixtures.
...
`scripts/benchmark_smoothing.py` isolates observation error by comparing each
filter's noisy-input output with a clean-input control across the frozen
motion-by-noise matrix. Keep the default seed, workloads, metric definitions,
and self-check thresholds stable within a minor release. If the protocol must
change:
...
The benchmark's throughput loop updates yaw, pitch, and roll only. Quality
values are deterministic; wall-clock speed is not.
