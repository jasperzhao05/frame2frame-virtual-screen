...
- A recorded Desmos application-content demonstration using attributed
  prerecorded person footage, with capture and rendering details.

- A reproducible 20-seed smoothing protocol with per-trial evidence, source
  hashes, seed variation, and same-frame screen error that retains causal lag.
...
- A project-authored five-policy filter visual and shared frozen registry for
  Raw, benchmark-only EMA 2.5 Hz, FIR angular default, One Euro angular
  default, and Kalman A100.

### Changed

- Smoothing receipt schema 5 adds same-frame error while preserving schema 4
  inputs and existing quality metrics. Repeated trials validate arithmetic
  without applying a single seed's quality thresholds to other seeds.
...
- Moved the model-free filter benchmark into distance-4 screen space and
  replaced its single periodic-noise trace with a balanced two-motion by
  four-noise non-periodic stress matrix, equal-cell macro and p95 ratios,
  worst-cell reduction, Screen Age, aligned motion error, EMA, and Kalman.
- Consolidated the two synthetic demo paths into one five-policy showcase;
  the pipeline benchmark now owns its private synthetic source directly.

### Fixed

- FIR tail alignment when an offline tracking segment is shorter than the
  designed delay, including EOF and reset. Full endpoint padding now matures
  every retained frame before emission.
